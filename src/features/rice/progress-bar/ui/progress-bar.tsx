    ...
    return (
        <div className="z-10 min-w-[80vw]">
            <div className={`flex justify-between mb-2 text-xs uppercase font-semibold ${!canTap ? 'text-gray-500' : 'text-[var(--main-color)]'}`}>
                <span>{per_period} / {RICE_PER_PERIOD}</span>
                <span>
                    {isPeriodActive ? formatTime(timeLeft) : (!canTap ? t("rice.progress_bar.title_negative") : t("rice.progress_bar.title_positive"))}
                </span>
            </div>
                <div className="w-full h-3 bg-font-color/10 rounded-[10px] overflow-hidden shadow-inner">
                <div className="h-full rounded-[10px] transition-all duration-300 shadow-[0_0_10px_var(--main-color)]"
                style={{
